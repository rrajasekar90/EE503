...
training_data = csvread('Zoo_Train.txt');
training_data_size = size(training_data,1);
test_data = csvread('Zoo_Test.txt');
...
for iter =1:training_data_size
    if training_data(iter,18) == 1
        train_class1 = cat(1,train_class1,training_data(iter,2:17));
    elseif training_data(iter,18) == 2
        train_class2 = cat(1,train_class2,training_data(iter,2:17));
    elseif training_data(iter,18) == 3
        train_class3 = cat(1,train_class3,training_data(iter,2:17));
    elseif training_data(iter,18) == 4
        train_class4 = cat(1,train_class4,training_data(iter,2:17));    
    elseif training_data(iter,18) == 5
        train_class5 = cat(1,train_class5,training_data(iter,2:17));
    elseif training_data(iter,18) == 6
        train_class6 = cat(1,train_class6,training_data(iter,2:17));
    elseif training_data(iter,18) == 7
        train_class7 = cat(1,train_class7,training_data(iter,2:17));
    end
end
...
for iter =1:test_data_size
    if test_data(iter,18) == 1
        test_class1 = cat(1,test_class1,test_data(iter,2:17));
    elseif test_data(iter,18) == 2
        test_class2 = cat(1,test_class2,test_data(iter,2:17));
    elseif test_data(iter,18) == 3
        test_class3 = cat(1,test_class3,test_data(iter,2:17));
    elseif test_data(iter,18) == 4
        test_class4 = cat(1,test_class4,test_data(iter,2:17));    
    elseif test_data(iter,18) == 5
        test_class5 = cat(1,test_class5,test_data(iter,2:17));
    elseif test_data(iter,18) == 6
        test_class6 = cat(1,test_class6,test_data(iter,2:17));
    elseif test_data(iter,18) == 7
        test_class7 = cat(1,test_class7,test_data(iter,2:17));
    end
end
...
for test_iter=1:test_data_size
    ...
    for iter=1:7
        if size(classMap(iter),1) ~= 0 
            likelihood_prob_row_given_class = normpdf(test_data(test_iter,2:17), mean_all_attr_in_class(iter),std_all_attr_in_class(iter));
...
accuracy_prediction = (index_of_class == test_data(:,18));
...
for attr=1:16
    ...
    for iter=1:7
        if size(classMap(iter),1) ~= 0
            ...
            P_attr_given_class = max(normpdf(test_data((2:length_of_class(iter)),attr+1), mean_of_attrs(attr),std_of_attrs(attr)));
...
attr_names = {'hair','feathers','eggs','milk','airborne','aquatic','predator','toothed','backbone','breathes','venomous','fins','legs','tail','domestic','catsize'};
for i=1:16
